...
using Zen.IdentityServer.Logging.Models;
...
internal class AuthorizeRequestValidator : IAuthorizeRequestValidator
{
    ...
    private async Task<AuthorizeRequestValidationResult> ValidateOptionalParametersAsync(ValidatedAuthorizeRequest request)
    {
        ...
        // check domain_hint
        //////////////////////////////////////////////////////////
        var domainHint = request.Raw.Get(OidcConstants2.AuthorizeRequest.DomainHint);
        if (domainHint.IsPresent())
        {
            if (domainHint.Length > _options.InputLengthRestrictions.DomainHint)
            {
                LogError("Domain hint too long", request);
                return Invalid(request, description: "Invalid domain_hint");
            }
